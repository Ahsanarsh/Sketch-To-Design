...
export interface MoodBoardImage {
    ...
    uploading: boolean
    error?: string
    url?: string | null // Convex URL for uploaded images
            ...
            if (currentImages.length === 0) {
                ...
                serverImages.forEach((serverImg) => {
                    ...
                    if (clientIndex !== -1) {
                        ...
                        if (mergedImages[clientIndex].preview && mergedImages[clientIndex].preview.startsWith('blob:')) {
    ...
    const removeImage = async (imageId: string) => {
        ...
        const updatedImages = images.filter((img) => {
            if (img.id === imageId) {
                ...
                if (!img.isFromServer && img.preview && img.preview.startsWith('blob:')) {
